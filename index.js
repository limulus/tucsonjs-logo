...
module.exports = function (svgRoot) {
  ...
  var clippedDrawingArea = root.clippedDrawingGroup();
...
var TucsonJSText = function (svg, colors) {
  ...
  // These paths are generated with TextToSVGPath. See:
  // https://github.com/limulus/TextToSvgPath
  // The font used is "Relay".
  // The typeface for "TUCSON" is "Condensed Black"
  // The typeface for "JS" is "Compressed Bold"
  ...
  // Found manually by running .getBBox().width on the <path> elments.
  this._tucsonWidth = 468.8;
  this._jsWidth = 165;
  
  // The gap between TUCSON and JS. Expressed in the units of the text SVGs,
  // not the root SVG of the logo.
  this._gap = 18;
};
...
TucsonJSText.prototype.draw = function (colors) {
  ...
  var combinedWidth = zoomFactor * (this._tucsonWidth + this._jsWidth + this._gap);
  var tucsonXOffset = (viewportDimensions.width - combinedWidth) / 2;
  var jsXOffset = tucsonXOffset + gap + this._tucsonWidth * zoomFactor;
  
  var zoomedViewBox = "0 0 " + zoomDivisor + " " + zoomDivisor;
  var tucsonContainer = TucsonJSText.textContainerForPath(this._tucson, this._colors[1]);
  this._svgRoot.addAt(tucsonContainer, tucsonXOffset, this._yOffset);
  ...
  this._svgRoot.addAt(jsContainer, jsXOffset, this._yOffset);
};
